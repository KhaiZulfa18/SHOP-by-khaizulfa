...
function tampil_data(pageno){
	...
	$.ajax({
		type: 'POST',
		url: base_url+'dashboard/get_checkout',
		data: {
			status: status,
			cari: cari,
			page: pageno
...
function paging(pageno){
	...
	$.ajax({
		type: "POST",		
		url: base_url+'dashboard/paging_checkout',
		data: {
			status: status,
			cari: cari,
			page: pageno
...
function lihat(id_checkout,id_invoice){
	var base_url = $('#base_url').val();
	var id_checkout = id_checkout;

	$.ajax({
		type: "POST",		
		url: base_url+'dashboard/lihat_data_invoice',
		data: {
...
function topdf(id){
	...
	$("#pdf-teks").html("<h6>Download atau Lihat PDF "+id+"?</h6>");
	$('#id_ch').val(id);
}
